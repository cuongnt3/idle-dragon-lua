...
--- @class UIFormationArenaTeamView : UIBaseView
UIFormationArenaTeamView = Class(UIFormationArenaTeamView, UIBaseView)

--- @return void
--- @param model UIFormationArenaTeamModel
function UIFormationArenaTeamView:Ctor(model, ctrl)
	...
	---@type List
	self.listTeamFormation = List()

	self.cacheWorldFormation = false
	UIBaseView.Ctor(self, model, ctrl)
	--- @type UIFormationArenaTeamModel
	self.model = model
end

--- @return void
function UIFormationArenaTeamView:OnReadyCreate()
	---@type UIFormation2Config
	...
	local onChangeSelectTeam = function(indexTab, lastTab)
		if lastTab ~= nil then
			...
			local button = self.config.teamArena:GetChild(lastTab - 1):GetComponent(ComponentName.UnityEngine_UI_Button)
			UIUtils.SetInteractableButton(button, true)
		end
		if indexTab ~= nil then
			---@type UnityEngine_Transform
			local child = self.config.teamArena:GetChild(indexTab - 1)
			---@type UnityEngine_UI_Button
	...
	local onClickSelectTeam = function(indexTab, lastTab)
		self:ChangeArenaTeam(indexTab)
	end

	self.tabTeam = UISelect(self.config.teamArena, nil, nil, onChangeSelectTeam, onClickSelectTeam, nil, nil)
...
--- @return void
function UIFormationArenaTeamView:InitButtonListener()
...
--- @return void
function UIFormationArenaTeamView:InitLocalization()
	...
	local localizeBattle = LanguageUtils.LocalizeCommon("save")
	...
	local localizeTeam = LanguageUtils.LocalizeCommon("team")
	self.config.localizeTeam1.text = localizeTeam
	self.config.localizeTeam2.text = localizeTeam
	self.config.localizeTeam3.text = localizeTeam
end

--- @return void
function UIFormationArenaTeamView:OnReadyShow(result)
...
--- @return void
function UIFormationArenaTeamView:OnFinishAnimation()
	UIBaseView.OnFinishAnimation(self)
	self:CheckAndInitTutorial()
end

--- @start world
function UIFormationArenaTeamView:CheckCanSkip(canSkip)
...
--- @start world
function UIFormationArenaTeamView:InitWorldFormation()
...
function UIFormationArenaTeamView:OnShowWorldFormation()
...
function UIFormationArenaTeamView:EnableDefenderTeam(isEnable)
	self.worldFormation:EnableDefenderTeamView(isEnable)
end
--- @end world

--- @param isEnable boolean
function UIFormationArenaTeamView:ShowListHero(isEnable)
...
--- @param isEnable boolean
function UIFormationArenaTeamView:IsFormationChange()
	local change = false
	---@param v TeamFormationInBound
	for i, v in ipairs(self.listTeamFormation:GetItems()) do
		if v:Equal(self.listTeamFormationOrigin:Get(i)) == false then
			change = true
			break
		end
	end
...
function UIFormationArenaTeamView:_Init(result)
	self.cacheWorld = nil
	self.cacheWorldFormation = false
	if result ~= nil then
		self.tabTeam:Select(self.tabTeam.indexTab)
		if result.cache == nil then
			self.listTeamFormationOrigin = result.listTeamFormation
			self.listTeamFormation = List()
			for i, v in ipairs(result.listTeamFormation:GetItems()) do
				self.listTeamFormation:Add(TeamFormationInBound.Clone(v))
			end
			self.model.teamFormation = self.listTeamFormation:Get(1)
			self.tabTeam:Select(1)
			self.model.defenderTeamInfo = result.battleTeamInfo
			self.callbackPlayBattle = result.callbackPlayBattle
			self.listHeroIgnor = ClientConfigUtils.GetListHeroIgnorByListTeam(self.listTeamFormation, 1)
		end
		...
		--else
		--	self.config.localizeBattle.text = LanguageUtils.LocalizeCommon("save")
		--end
...
function UIFormationArenaTeamView:Hide()
...
function UIFormationArenaTeamView:HideWorldFormation()
...
--- @return void
function UIFormationArenaTeamView:UpdatePowerAttacker()
	---@type BattleTeamInfo
...
--- @return void
function UIFormationArenaTeamView:ChangeTeam()
...
--- @return void
function UIFormationArenaTeamView:ChangeArenaTeam(indexTab)
	self.model.teamFormation = self.listTeamFormation:Get(indexTab)
	self.tab:Select(self.model.teamFormation.formationId)
	self.worldFormation:SetAttackerFormation(self.model.teamFormation.formationId)
	---@type DetailTeamFormation
	local detailTeamFormation = DetailTeamFormation.CreateByTeamFormationInBound(self.model.teamFormation)
	self.worldFormation:ShowAttackerPredefineTeam(detailTeamFormation)
	self.worldFormation:ShowAttackerSummoner(self.model.teamFormation.summonerId, self.summonerInbound.star)
	self.listHeroIgnor = ClientConfigUtils.GetListHeroIgnorByListTeam(self.listTeamFormation, indexTab)
	self.heroList:ReturnPool()
	self.heroList:SetData(InventoryUtils.Get(ResourceType.Hero), self.canPlayMotion)
	self:UpdateTeamFormation()
end
...
--- @return void
--- @param heroResource HeroResource
function UIFormationArenaTeamView:AddHeroToTeam(heroResource)
...
function UIFormationArenaTeamView:RemoveHeroFromTeam(inventoryId)
...
function UIFormationArenaTeamView:CallbackSwapHero(hero1, hero2)
	self.model.teamFormation:SwapHero(hero1, hero2)
	self:UpdateTeamFormation()
end

--- @param hero {isFrontLine, positionId}
function UIFormationArenaTeamView:CallbackRemove(hero)
	self.model.teamFormation:RemoveHeroPosition(hero)
	self.heroList.uiScroll:RefreshCells()
	self:UpdateTeamFormation()
end

function UIFormationArenaTeamView:CallbackSelectSummoner()
	if self.summonerInbound.star > 3 then
		if self.model.teamFormation:IsContainHeroInFormation() then
			...
			data.callbackSwitch = function(summonerId)
				...
				if self.model.currentMode ~= nil
					...
					self:SaveFormationServer(function()
						PopupMgr.ShowAndHidePopup(UIPopupName.UIFormationArenaTeam, dataFormation, UIPopupName.UISwitchCharacter)
					end)
				else
					PopupMgr.ShowAndHidePopup(UIPopupName.UIFormationArenaTeam, dataFormation, UIPopupName.UISwitchCharacter)
				end
			...
			self.cacheWorldFormation = false
			self.cacheWorld = self.cacheWorldFormation
			PopupMgr.ShowAndHidePopup(UIPopupName.UISwitchCharacter, data, UIPopupName.UIFormationArenaTeam)
		else
...
function UIFormationArenaTeamView:SetListLinking(listHero, rectTransform, listLinkingReturn)
...
--- @return void
function UIFormationArenaTeamView:UpdateLinkingTeamFormation()
	---@type List
...
--- @return void
function UIFormationArenaTeamView:UpdateLinkingDefenderTeam()
	---@type List
...
--- @return void
function UIFormationArenaTeamView:UpdateTeamFormation()
...
--- @return void
function UIFormationArenaTeamView:UpdateCompanionBuff()
...
function UIFormationArenaTeamView:OnClickLinkingAttacker(index)
...
function UIFormationArenaTeamView:OnClickLinkingDefender(index)
...
function UIFormationArenaTeamView:OnClickCompanion1()
	local data = {}
...
function UIFormationArenaTeamView:OnClickCompanion2()
	local data = {}
...
function UIFormationArenaTeamView:OnClickSkip()
	self:OnClickBattle(true)
end

---@return void
function UIFormationArenaTeamView:SaveFormationLocal()
...
function UIFormationArenaTeamView:SaveFormationServer(onSuccess, showWaiting)
	if self.model.currentMode ~= nil
			and self.model.currentMode ~= GameMode.ARENA
			and self.model.currentMode ~= GameMode.GUILD_WAR then
...
---@return void
function UIFormationArenaTeamView:OnClickBattle(isSkip)
	if self.model.teamFormation ~= nil and self.model.teamFormation:IsContainHeroInFormation() then
		...
		if self.callbackPlayBattle ~= nil then
			...
			--end
			self.callbackPlayBattle(self.listTeamFormation, callbackSuccess, self.powerAttacker)
		end
...
---@param step number
function UIFormationArenaTeamView:ShowTutorial(tutorial, step)
	--XDebug.Log(step)
...
--- @param bgParams {}
function UIFormationArenaTeamView:ShowBgAnchor(bgParams)
...
function UIFormationArenaTeamView:OnClickRemoveAll()
	---@param v HeroFormationInBound
	for i, v in pairs(self.model.teamFormation.backLine:GetItems()) do
