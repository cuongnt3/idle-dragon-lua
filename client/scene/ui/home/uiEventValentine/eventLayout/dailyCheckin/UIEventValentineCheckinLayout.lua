require "lua.client.scene.ui.home.uiEventValentine.eventLayout.dailyCheckin.DailyRewardValentinePageView"
--- @class UIEventValentineCheckinLayout : UIEventValentineLayout
UIEventValentineCheckinLayout = Class(UIEventValentineCheckinLayout, UIEventValentineLayout)

local TIME_PAGE_MOVE = 0.3
local RANGE_MOVE = 100
local DAY_PER_PAGE = 7

function UIEventValentineCheckinLayout:Ctor(view, xmasTab, anchor)
    ...
    --- @type number
    self.currentPage = 1
    --- @type DailyRewardValentinePageView
    ...
    UIEventValentineLayout.Ctor(self, view, xmasTab, anchor)
    self:SetUpButtonPage()
end

function UIEventValentineCheckinLayout:SetUpButtonPage()
...
function UIEventValentineCheckinLayout:OnShow()
...
function UIEventValentineCheckinLayout:OnHide()
...
function UIEventValentineCheckinLayout:InitLayoutConfig()
    local inst = PrefabLoadUtils.Instantiate("valentine_checkin", self.anchor)
...
function UIEventValentineCheckinLayout:GetCurrentPage()
    if self.page1 == self.currentPageView then
        return self.page1
    end
    return self.page2
end
function UIEventValentineCheckinLayout:GetCurrentAndNext()
    local current = nil
    local next = nil
    if self.page1 == self.currentPageView then
        current = self.page1
...
function UIEventValentineCheckinLayout:InitTimeView()
...
function UIEventValentineCheckinLayout:FillReward()
...
function UIEventValentineCheckinLayout:SetDefaultPosition()
...
function UIEventValentineCheckinLayout:UpdateButtons()
...
function UIEventValentineCheckinLayout:NextPage()
    local touch = TouchUtils.Spawn("Valentine.NextPage")
...
function UIEventValentineCheckinLayout:BackPage()
    local touch = TouchUtils.Spawn("Valentine.BackPage")
    self.currentPage = self.currentPage - 1
    self.uiSelectPage:Select(self.currentPage)
    self:UpdateButtons()
    local current, next = self:GetCurrentAndNext()
...
function UIEventValentineCheckinLayout:InitTileDay()
    ---@type DailyRewardPageView
    local onClickClaim = function(day, require, isFree)
        self:OnClickClaim(day, require, isFree)
    end
    self.page1 = DailyRewardValentinePageView(self.layoutConfig.content1, onClickClaim, self.isNewLogin)
    ---@type DailyRewardValentinePageView
    self.page2 = DailyRewardValentinePageView(self.layoutConfig.content2, onClickClaim, self.isNewLogin)
end

function UIEventValentineCheckinLayout:InitButtons()
...
function UIEventValentineCheckinLayout:InitLocalization()
    self.layoutConfig.localizeClaim.text = LanguageUtils.LocalizeCommon("claim")
    self.layoutConfig.eventTittle.text = LanguageUtils.LocalizeCommon("valentine_checkin_title")
    self.layoutConfig.nextFreeText.text = LanguageUtils.LocalizeCommon("next_free")
end

---@param require RewardInBound
function UIEventValentineCheckinLayout:OnClickClaim(day, require, isFree)
    ...
    NetworkUtils.Request(OpCode.EVENT_VALENTINE_LOGIN_CLAIM, nil, onReceived)
end

--- @param rewardList List
function UIEventValentineCheckinLayout:ShowAndAddReward(rewardList)
    local touchObject = TouchUtils.Spawn("Valentine.Login")
...
function UIEventValentineCheckinLayout:UpdateViewOldLogin()
...
function UIEventValentineCheckinLayout:UpdateViewNewLogin()
    local current = self:GetCurrentPage()
...
function UIEventValentineCheckinLayout:UpdateView()
...
function UIEventValentineCheckinLayout:EnableTimeCountDown(isEnable, index, nextReward)
...
function UIEventValentineCheckinLayout:StartUpdateTime()
...
function UIEventValentineCheckinLayout:SetTimeRefresh()
...
function UIEventValentineCheckinLayout:RemoveUpdateTime()
